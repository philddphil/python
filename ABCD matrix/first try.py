...
zs0, qs0, ns0 = prd.ABCD_propagate(q0, z1, res=20)
...
zs4, qs4, ns4 = prd.ABCD_propagate(q4, z5, z_start=z4, res=20)
_, ps4, _ = prd.ABCD_propagate(p4, z5, z_start=z4, res=20)
...
# print('θ final', np.round(180 * θs[-1] / π, 2))

fig1 = plt.figure('fig1')
ax1 = fig1.add_subplot(1, 1, 1)
fig1.patch.set_facecolor(cs['mdk_dgrey'])
ax1.set_xlabel('optical axis (μm)')
...
plt.plot(1e6 * zs, 1e6 * ws, '.-', c=cs['ggred'], label='Gaussian Beam')
plt.plot(1e6 * zs, -1e6 * ws, '.-', c=cs['ggred'])
plt.plot(1e6 * zs, 1e6 * xs, '-', c=cs['ggblue'], label='Raytrace')
plt.plot(1e6 * zs, -1e6 * xs, '-', c=cs['ggblue'])
plt.plot(1e6 * zs, 1e1 * ns, '-', c=cs['mdk_purple'])

# plt.plot([1e6 * z2, 1e6 * z2],  [220, -
#                                  220], '-', c=cs['mdk_orange'])
# plt.plot([1e6 * z3, 1e6 * z3],  [1e6 * np.max(ws_alt), -
#                                  1e6 * np.max(ws_alt)], '-', c=cs['mdk_pink'])
