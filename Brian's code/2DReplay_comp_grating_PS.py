...
crosstalk_matrix = np.zeros([qq1+1,qq2+1])
signal_matrix = np.zeros([qq1+1,qq2+1])

for count1 in xrange(qq1+1):
    shift = shift_min + count1*(shift_max-shift_min)/(1.0*qq1)
    for count2 in xrange(qq2+1):
        weight = weight_min+count2*(weight_max-weight_min)/(1.0*qq2)

        phase_SLM = zeros([N_pixels,N_pixels])
        grey_SLM = zeros([N_pixels,N_pixels])

        for n in xrange(N_pixels):
            for m in xrange(N_pixels):
                ...
                p1c = weight * np.sin(2 * pi *((xSLM/(p2*pixel) + ySLM/(p2*pixel)) + shift))
        ...
        for ii in xrange(NT):
            for jj in xrange(NT):
                if ((ii/res) == floor(ii/res)) and ((jj/res) == floor(jj/res)):
                    Np[ii-res+1+res/2,jj-res+1+res/2] = phase_SLM[ii/res, jj/res]
        ...
        phase_SLM_1 = fftshift(fft2(fftshift(phase_SLM)))*fftshift(fft2(fftshift(psf)))
        ...
        for n in xrange(NT):
            for m in xrange(NT):
                ...
                # Phase step according to modulo 2pi algorithm
                diff_phase_L = 2*pi*((phase_L/(2*pi))-floor(phase_L/(2*pi)))
        ...
        # Define zero padding factor, pad, and generate associated replay field calaculation matrices 
        pad = 4
        ...
        E_replay_zero = fftshift(fft2(fftshift(E_calc_amplt*np.exp(1j*E_calc_phase))))
        ...
        for ii in xrange(NT*pad):
            pos = ((1+NT*pad/2)-ii)*f*wavelength/((pixel/res)*pad*NT)
            x_vec[ii] = pos*1e6
        ...
        Replay_zero = abs(E_replay_zero[xcen-range:xcen+range, ycen-range:ycen+range])

        [xpos, ypos] = np.where(I_replay==I_replay.max())
        print 'Signal position', xpos, ypos
        Replay_sig = abs(E_replay[xpos-range:xpos+range, ypos-range:ypos+range])

        xxt = xcen-(xpos-xcen)
        yxt = ycen-(ypos-ycen)
        print '-1 order rosstalk position', xxt, yxt
        Crosstalk_sig = abs(E_replay[xxt-range:xxt+range, yxt-range:yxt+range])

        points = 2*range


        # Alternative calculation
        test = (np.sum(((Replay_sig)*(Replay_zero))))**2/((np.sum(np.abs(E_replay)**2))*(np.sum(np.abs(E_replay_zero)**2)))
        print 'test signal = ', test
        signal_matrix[count1, count2] = test*100

        test = (np.sum(((Crosstalk_sig)*(Replay_zero))))**2/((np.sum(np.abs(E_replay)**2))*(np.sum(np.abs(E_replay_zero)**2)))
        print 'test crosstalk = ', 10*log10(test)
        crosstalk_matrix[count1, count2] = 10*log10(test)

        I1_final = zeros([200, 200])
        I1_final = 10*np.log10(I_replay_zero[NT*pad/2-100:NT*pad/2+100, NT*pad/2-100:NT*pad/2+100])
        I1_final[I1_final < -50] = -50

        I2_final = zeros([200, 200])
        I2_final = 10*np.log10(I_replay[NT*pad/2-100:NT*pad/2+100, NT*pad/2-100:NT*pad/2+100])
...
figure(1)
imshow(signal_matrix, interpolation = 'nearest', extent=[weight_min,weight_max,shift_min,shift_max])
xlabel('Relative weight of sin grating')
ylabel('Grating shift (Radians)')
title('Signal matrix (period of 10 pixels)')
colorbar()

figure(2)
imshow(crosstalk_matrix, interpolation = 'nearest', extent=[weight_min,weight_max,shift_min,shift_max])
xlabel('Relative weight of sin grating')
ylabel('Grating shift (Radians)')
title('Crosstalk matrix (period of 10 pixels)')
colorbar()
